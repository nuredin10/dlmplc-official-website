...
$(document).on('ready', function () {
	...
	var content = $('#anim-text').text().split('');
$('#anim-text').html('');
	function myTimer() {		if(content[i]){
		// console.log(content);
		var cur = $("#anim-text").html();
$('#anim-text').html(cur+content[i]);
		i++;
	} else {
		i=0;		$('#anim-text').html('');
	}
	...
	var i = 0;
	var inter= setInterval(myTimer,90);
	...
	showSiteBtn.on('click', function () {
		contactSwiper.slideTo($('.site-map').index(), 300, false);
		return false;
	});
	...
	// 11. fabrics
	var showCottonfib = $('.fib-cotton');
	var showOrganicfib = $('.fib-organic');
	var showSilkfib = $('.fib-silk');
	var showLinenfib = $('.fib-linen');
	var showSiteBtnfib = $('.fib-map');
	var showTencelfib = $('.fib-tencel');
	var showModelfib = $('.fib-model');
	var showRecyclefib = $('.fib-recycle');
	var fibSwiper = new Swiper('.slider-swiper-fib .swiper-container', {
		grabCursor: true,
		centeredSlides: false,
		autoplay: 0,
		autoplayDisableOnInteraction: false,
	...
	showCottonfib.on('click', function () {
		fibSwiper.slideTo($('.fib-cotton-slide').index(), 300, false);
		return false;
	});
	showOrganicfib.on('click', function () {
		fibSwiper.slideTo($('.fib-organic-slide').index(), 300, false);
		return false;
	});
	showLinenfib.on('click', function () {
		fibSwiper.slideTo($('.fib-linen-slide').index(), 300, false);
		return false;
	});
	showSilkfib.on('click', function () {
		fibSwiper.slideTo($('.fib-silk-slide').index(), 300, false);
		return false;
	});
	showTencelfib.on('click', function () {
		fibSwiper.slideTo($('.fib-tencel-slide').index(), 300, false);
		return false;
	});
	showModelfib.on('click', function () {
		fibSwiper.slideTo($('.fib-model-slide').index(), 300, false);
		return false;
	});
	showRecyclefib.on('click', function () {
		fibSwiper.slideTo($('.fib-recycle-slide').index(), 300, false);
		return false;
	});
	// 12. Dyes
	var showInfoBtndyes = $('.natural-dyes');
	var showMessageBtndyes = $('.dyes');
	var dyesSwiper = new Swiper('.slider-swiper-dyes .swiper-container', {
		grabCursor: true,
		centeredSlides: false,
		autoplay: 0,
		autoplayDisableOnInteraction: false,
	...
	showMessageBtndyes.on('click', function () {
		dyesSwiper.slideTo($('.dyes-message-slide').index(), 300, false);
		return false;
	});
	showInfoBtndyes.on('click', function () {
		dyesSwiper.slideTo($('.dyes-information-slide').index(), 300, false);
		return false;
	});
